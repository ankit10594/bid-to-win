...
function calculate() {
  ...
  if (dataList != null) {
    ...
    if (dataJson.length >= 5) {
      ...
      //If unique value occured only once
      if (occuredOnlyOnce.length != 0) {
        //sort asc order with respect to timestamp
        let uniqueValueBidFirst = occuredOnlyOnce.sort(
          (a, b) =>
            parseFloat(occuredOnlyOnce.timestamp) -
            parseFloat(occuredOnlyOnce.timestamp)
        );
        ...
        $("#winnerDiv h4").html(
          "Winner is " +
            uniqueValueBidFirst[0].name +
            " with bid value " +
            uniqueValueBidFirst[0].bid
        );
      } else {
        ...
        if (occursAlltheTime.length != 0) {
          ...
          let ascOrderTimeStamp = occursAlltheTime.sort(
            (a, b) =>
              parseFloat(occursAlltheTime.timestamp) -
              parseFloat(occursAlltheTime.timestamp)
          );
          ...
          $("#winnerDiv h4").html(
            "Winner is " +
              takeOnlyFirstValueWithoutDuplicate[0].name +
              " with bid value " +
              takeOnlyFirstValueWithoutDuplicate[0].bid
          );
